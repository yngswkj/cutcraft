...
CutCraft は、Sora 2 / Veo API を活用して約2分間の動画を制作するための個人用ワークフローツール。AI による設計図提案からシーン別動画生成までを一気通貫で管理する。
...
| 項目 | 内容 |
|------|------|
| 画像生成 | DALL-E 3 で参考画像を生成（1792×1024、HD品質） |
...
| 項目 | 内容 |
|------|------|
| Sora 生成 | OpenAI Sora API で動画生成（5/10/15/20秒） |
| Veo 生成 | Google Veo 2 API で動画生成（5〜8秒） |
| 一括生成 | 未生成シーンを一括で動画生成 |
...
| 項目 | 内容 |
|------|------|
| ローカル画像入力 | DALL-E 3 生成ではなくユーザー手持ちの画像をアップロード |
...
| 変数名 | 用途 |
|--------|------|
| `OPENAI_API_KEY` | OpenAI API（GPT-4o, DALL-E 3, Sora） |
| `GOOGLE_AI_API_KEY` | Google Gemini API（Veo 2） |
